...
//vt+1/2at^2
  ...
  auto look_ahead_distance = velocity_mag * _lookahead_time + (0.5 * accel_mag * _lookahead_time * _lookahead_time );   // <- Fixed This
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      goal.velocity.x = _speed_limit * std::cos(goal.rotation.yaw);  // <- Fixed This
      goal.velocity.y = _speed_limit * std::sin(goal.rotation.yaw);  // <- Fixed This
      goal.velocity.z = 0;
    }
